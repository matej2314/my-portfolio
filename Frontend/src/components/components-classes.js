
export const compClasses = {
    contactForm: {
        formDiv: "w-full h-dvh text-gray-100 flex flex-col items-center rounded-md pt-2 pb-2 indirect:pb-14 md:pt-24 overflow-scroll no-scrollbar z-10",
        contactForm: "w-full h-full px-4 indirect:px-3 md:w-1/3 flex flex-col items-center justify-between font-black overflow-scroll no-scrollbar",
        h2: "text-2xl indirect:text-5xl indirect:mb-4",
        h3: "indirect:text-2xl text-lg text-lime-500",
        firstLabel: "indirect:text-lg indirect:mb-2 indirect:mt-4",
        otherLabels: "indirect:text-lg indirect:my-2",
        input: "w-full text-stone-950 p-1 indirect:p-3 md:p-2",
        textarea: "w-full text-sm indirect:text-lg text-stone-950 p-3 md:p-2 resize-none",
        submitButton: "w-fit h-fit indirect:p-4 bg-[#61751b] rounded-md indirect:mt-2 md:mt-4 hover:bg-[#374528]"
    },
    footer: {
        wrapper: "w-[100%] flex flex-row justify-center items-center mx-auto",
        footer: "w-full bg-neutral-600/30 h-10 text-md rounded-md flex flex-row justify-center items-center px-2",
        paragraph: "w-full flex items-center justify-center md:justify-start text-gray-300",
        span: "w-fit hidden md:inline text-[#6f963b]",
    },
    leftSidebar: {
        leftSidebar: "h-content w-fit ml-10 flex flex-col justify-between gap-3",
        link: "bg-[#97d93f] w-[84.5%] rounded-lg flex justify-center items-center py-5 px-5 text-black hover:bg-[#374528] hover:text-slate-200",
        span: "font-bold font-sans tracking-wider",
    },
    mainLeftContainer: {
        mainContainer: 'bg-transparent z-0 w-full h-full mr-5 flex flex-col items-center rounded-md text-gray-300 z-0',
    },
    mainRightContainer: {
        mainContainer: 'bg-neutral-600/30 text-gray-300 w-full h-full rounded-md overflow-scroll no-scrollbar',
    },
    menu: {
        menu: "w-[84%] h-[102%] bg-neutral-600/30 flex flex-col justify-around items-center rounded-lg",
        ul: "flex flex-col items-center gap-8 text-gray-300 text-lg",
        li: "w-fit h-fit",
        link: "flex justify-center min-h-[1.5em]",
    },
    portfolio: {
        wrapper_nested: "w-full h-full flex flex-col gap-4 px-2.5",
        wrapper: "w-full h-full flex flex-col gap-4 px-10",
    },
}

